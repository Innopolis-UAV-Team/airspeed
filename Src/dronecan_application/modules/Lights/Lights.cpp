...
LightsModule::LightsModule()
    : int_led_driver(GPIOPin::INT_RGB_LED_RED, GPIOPin::INT_RGB_LED_GREEN,
                     GPIOPin::INT_RGB_LED_BLUE),
      ext_led_driver(PwmPin::PWM_4, PwmPin::PWM_3, PwmPin::PWM_6) {
    update_params();
    instance_initialized = true;

    auto sub_id = uavcanSubscribe(
        UAVCAN_EQUIPMENT_INDICATION_LIGHTS_COMMAND_SIGNATURE,
        UAVCAN_EQUIPMENT_INDICATION_LIGHTS_COMMAND_ID, callback);
    if (sub_id < 0) {
...
void LightsModule::reset_command() { command = {}; }

void LightsModule::spin_once() {
    Rgb565Color color = _default_color;
    if (HAL_GetTick() < ttl_current_cmd_ms) {
        color = Rgb565Color{command.color.red, command.color.green,
                            command.color.blue};
        ...
        if (toggle_type == 2) {
            uint8_t intensity =
                max_intensity *
                ((HAL_GetTick() % toggle_period_ms) / float(toggle_period_ms));
...
void LightsModule::publish_command() {
    ...
    if (next_pub_ms < HAL_GetTick()) {
        ...
        dronecan_equipment_indication_lights_command_publish(&cmd,
                                                             &transfer_id);
        transfer_id++;
    }
}
...
void LightsModule::callback(CanardRxTransfer* transfer) {
    ...
    int8_t res = dronecan_equipment_indication_lights_command_deserialize(
        transfer, &raw_command);
    if (res > 0) {
...
void LightsModule::update_params() {
    auto default_color =
        paramsGetIntegerValue(IntParamsIndexes::PARAM_LIGHTS_DEFAULT_COLOR);

    toggle_type = paramsGetIntegerValue(IntParamsIndexes::PARAM_LIGHTS_TYPE);
    max_intensity =
        paramsGetIntegerValue(IntParamsIndexes::PARAM_LIGHTS_MAX_INTENSITY);
    light_id = paramsGetIntegerValue(IntParamsIndexes::PARAM_LIGHTS_ID);
    toggle_period_ms =
        paramsGetIntegerValue(IntParamsIndexes::PARAM_LIGHTS_PERIOD_MS);

    if (toggle_type == 1) {
        duty_cycle_ptc = paramsGetIntegerValue(
            IntParamsIndexes::PARAM_LIGHTS_DUTY_CYCLE_PTC);
    ...
    _default_color =
        Rgb565Color::from_rgb_simple_color(RgbSimpleColor(default_color));
...
void LightsModule::apply_params() {
    int_led_driver =
        GPIORgbLedDriver(GPIOPin::INT_RGB_LED_RED, GPIOPin::INT_RGB_LED_GREEN,
                         GPIOPin::INT_RGB_LED_BLUE);
    ...
    ext_led_driver =
        PwmRgbLedDriver(PwmPin::PWM_4, PwmPin::PWM_3, PwmPin::PWM_6);
